    ...
    if (level < max_nr)
    {
      ...
      for (int i = max_start+1; i < nr; i++)
      {
        ...
        if ((next[i]) && (!considered[i]))
        {
          ...
          new_added->group_ = added.group_; new_added->group_[i] = 1;
          ...
          Eigen::VectorXi new_next = next; new_next[i] = 0;
          if (level == 0){
          ...
          {
            for (int j = 0; j < nr; j++)
              new_next[j] = new_next[j] || connected(i,j); // check for already added things is implicitly handled by considered
