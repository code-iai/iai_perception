...
//empty constructor
EuclideanClusterExtractorCurvature::EuclideanClusterExtractorCurvature():normalsSet_(false),
    maxPts_(std::numeric_limits<int>::max())
{

}

void EuclideanClusterExtractorCurvature::segment()
{
    ...
    for(size_t i = 0; i < indices_rnd.size(); ++i)
    {
        ...
        if(seed_queue->indices.size() >= minPts_ && seed_queue->indices.size() <= maxPts_)
        {
            ...
            segments_.push_back(seed_queue);
        }
    }
